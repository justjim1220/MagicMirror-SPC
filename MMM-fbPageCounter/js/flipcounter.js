...
var flipCounter = function(d, options){
  ...
  this.incrementTo = function doIncrement(number, pace, cycles) {
    var curValue = counter.value
    ...
    var inc = Math.ceil((targetValue - curValue) / cycles)
      ...
      _digitCheck(curValue, counter.value);
      nextCount = setTimeout(function(){doIncrement(number, Math.ceil(pace * 1.01), cycles)}, pace);
    } else {
      counter.auto = false
    }
  }
...
/*
  this.incrementTo = function doIncrement(n, s, c){
    var val = counter.value,

    seconds = (typeof s == 'undefined') ? false : s
    ...
    if (val != n){
      ...
      y = counter.value;
      _digitCheck(x,y);
      nextCount = setTimeout(function(){doIncrement(n, seconds, cycles)}, (seconds/cycles * 1000));
